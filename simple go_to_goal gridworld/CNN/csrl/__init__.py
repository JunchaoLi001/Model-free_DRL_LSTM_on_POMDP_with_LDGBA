...
class ControlSynthesis:
    ...
    """
    def __init__(self, mdp, oa, discount=0.9, discountB=0.99):
        self.mdp = mdp
        self.oa = oa
        ...
        self.shape_Q = oa.shape + mdp.shape + (len(mdp.A),)
    ...
    # rescale obsv_state coordinates
    def state_rescale(self, obsv_state):
        q_state, n_rows, n_cols = self.shape[1], self.shape[2], self.shape[3]
        coord = (obsv_state[0]/n_rows, obsv_state[1]/n_cols,)
        return coord
    ...
    def train_DRQN(self,start=None, EPISODES=None, num_steps=None, batch_size=None, weights_update=None, state_sequence_size=None):
        ...
        agent = DRQNAgent(obsv_size, prod_action_size, state_sequence_size)
        
        ### if want to continue train the model
        #agent.load("./save/model_name.h5")
        
        num_episode_for_reward = 1 # print the accumulated reward per num of episode
        ...
        for e in range(EPISODES):
            ...
            mdp_state = (random.randint(8,9),random.randint(0,1))
            #while self.mdp.label[mdp_state[0],mdp_state[1]] == ('c',) or self.mdp.structure[(mdp_state[0],mdp_state[1])]=='B':
                #mdp_state = self.mdp.random_state()
            # convert to the start product state
            state = (self.shape[0]-1,self.oa.q0)+mdp_state
            print('START state: '+str(state))
            
            # --------------------------- one-hot-encoding the observed (state)  ----------------------------------- #
            obsv_states, obsv_probs = self.mdp.get_observation_prob(state[-2:])
            obsv_state = self.mdp.generate_obsv_state(obsv_states, obsv_probs)
            # encodng (state index)
            obsv_state_input = self.state_one_hot_encoding(obsv_state)
            
            # --------------------------- Append the NEXT (state), as the sequences ------------------------------- #
            next_state_sequence.append(obsv_state_input)
            if len(next_state_sequence) > state_sequence_size:
                next_state_sequence.pop(0)
            #print('len(next_state_sequence): ' +str(len(next_state_sequence)))
            #print('next_state_sequence: ' +str(next_state_sequence))
            #-------------------------------------------------------------------------------------------------------------------------#
            for step in range(num_steps):
                #print('STATE: '+str(state)+'###########')
                #print('obsv_state_input: '+str(obsv_state_input))
                
                # --------------------------- append the (state), as the sequences -------------------------------- #
                if eps_count == 0:
                    state_sequence.append(obsv_state_input)
                    if len(state_sequence) > state_sequence_size:
                        state_sequence.pop(0)
                    #print('len(state_sequence): ' +str(len(state_sequence)))
                    #print('state_sequence: ' +str(state_sequence))
                    
                # --------------------------- select the action from the approximated q values -------------------------------------- #
                q_values = agent.act(state_sequence)
                ...
                obsv_states, obsv_probs = self.mdp.get_observation_prob(next_state[-2:])
                # observe the next state
                obsv_state = self.mdp.generate_obsv_state(obsv_states, obsv_probs)
                # encodng (state index)
                obsv_state_input_next = self.state_one_hot_encoding(obsv_state)
                #print('obsv_state: '+str(obsv_state))
                #print('obsv_state_input_next: '+str(obsv_state_input_next))
                ...
                if eps_count == 0 and len(state_sequence)==state_sequence_size and len(next_state_sequence)==state_sequence_size:
                    ...
                    #print('reward'+str(reward))
                    #print('next_state_sequence: ' +str(next_state_sequence))
                    agent.memorize(state_sequence, action_input, reward, next_state_sequence)
                    #print('==========================')
                ...
                if step > 10 and self.mdp.label[state[2],state[3]]==('c',): # current state: trapping state
                    break
            ...
            if len(accumulated_rewards)>=num_episode_for_reward:
                ...
                exploration_rate = []
        agent.save("./save/Simple_go_to_goal_2d_cnn.h5")
    ...
    def verify_DRQN(self,start=None, EPISODES=None, num_steps=None, state_sequence_size=None):
        from dqn_rnn import DRQNAgent
        ...
        ### if want to continue train the model
        agent.load("./save/Simple_go_to_goal_2d_cnn.h5")
        
        num_episode_for_reward = 10 # print the accumulated reward per num of episode
        Path = np.zeros((EPISODES, num_steps))
        ...
        for e in range(EPISODES):
            ...
            for step in range(num_steps):
                ...
                # select the action from the approximated q values
                q_values = agent.act_trained(state_sequence)
                ...
                obsv_state_input_next = self.state_one_hot_encoding(obsv_state)
                #print('obsv_state: '+str(obsv_state))
                #print('obsv_state_input: '+str(obsv_state_input_next))
                ...
                Path[e][step] = self.path_state_coord(state)
