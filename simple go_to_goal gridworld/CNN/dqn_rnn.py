...
from tensorflow.keras.layers import Input, Flatten, Dense, Dropout, LSTM, concatenate, Conv2D
...
class DRQNAgent:
    def __init__(self, state_size, action_size, state_sequence_size):
        ...
        self.eval_model = self._build_model_CNN()
        self.tar_model = self._build_model_CNN()
    ...
    def _build_model_RNN(self):
        ...
        dense_1 = Dense(32, activation=None)(state_features)
        dense_2 = Dense(16, activation=None)(dense_1)
        dense_out = Dense(self.action_size, activation='relu')(dense_2)
    ...
    def _build_model_CNN(self):
        ...
        #model.add(tf.keras.layers.Conv1D(6, (2, 2), strides=(1, 1), activation='relu', input_shape=(1, self.state_sequence_size, self.state_size,)))
        model.add(Conv2D(6, kernel_size=(3, 3), strides=1, input_shape = (self.state_sequence_size, self.state_size,1)))
        model.add(Conv2D(12, kernel_size=(2, 2), strides=1))
    ...
    def act(self, state_sequence):
        ...
        if np.random.rand() <= self.epsilon or len(state_sequence) < self.state_sequence_size:
            ...
            state_sequence = state_sequence.reshape((1, self.state_sequence_size, self.state_size,1))
    ...
    def replay(self, num_episode, episode, batch_size):
        ...
        for i in range(len(minibatch)):
            ...
            arr_states_seq = arr_states_seq.reshape((1, self.state_sequence_size, self.state_size,1))
            arr_next_states_seq = arr_next_states_seq.reshape((1, self.state_sequence_size, self.state_size,1))
            target = arr_rewards
            if arr_rewards!=10.:
        ...
        arr_train_X_states = arr_train_X_states.reshape((len(arr_train_X_states), self.state_sequence_size, self.state_size,1))
